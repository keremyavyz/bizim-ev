...
def scrape_product_info(url):
    ...
    try:
        # Microlink API (En iyi resim bulucu)
...
with tabs[0]:
    with st.expander("➕ ÜRÜN EKLE (RESİM GARANTİLİ)", expanded=True):
        st.info("💡 İpucu: Site linkini yapıştırın. Resim gelmezse 2. kutuya resim linkini yapıştırın.")
        
        with st.form("add_item"):
            c1, c2 = st.columns([1, 1])
            url = c1.text_input("1. Ürün Linki")
            img_manual = c2.text_input("2. Resim Linki (Opsiyonel - Sağ Tık Kopyala)")
    ...
    if not view_df.empty:
        ...
        for i, (idx, row) in enumerate(view_df.iterrows()):
            with cols[i % 2]:
                ...
                # HTML KART (GİRİNTİSİZ - DÜZELTİLMİŞ KISIM BURASI)
                card_html = f"""
